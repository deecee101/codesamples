...
var browserok=ie4||ie5||ie||ns4||ns6
var coldivs = new Array();

function colordiv(div, obj, typ, fefx, gtype, speed){
	this.type = typ;
	this.obj = obj;
	...
	function fadebg() {
		...
		if (this.stepred!=0 || this.stepgreen!=0 || this.stepblue!=0) {
			...
			if (ie5 && this.gradient_effect!="none") {
				if (this.gradient_effect=="horizontal") {this.gradient_effect=1}
				if (this.gradient_effect=="vertical") {this.gradient_effect=0}
				this.greencol_1 = this.hexc[Math.floor(this.newred/16)];
				this.greencol_2 = this.hexc[this.newred%16];
				this.bluecol_1 = this.hexc[Math.floor(this.newgreen/16)];
				this.bluecol_2 = this.hexc[this.newgreen%16];
				this.redcol_1 = this.hexc[Math.floor(this.newblue/16)];
				this.redcol_2 = this.hexc[this.newblue%16];
				var newcolorCompl="#"+this.redcol_1+this.redcol_2+this.greencol_1+this.greencol_2+this.bluecol_1+this.bluecol_2
				if(this.type == 'bg'){
					document.getElementById(div).style.filter=
	"progid:DXImageTransform.Microsoft.Gradient(startColorstr="+this.newcolorCompl+", endColorstr="+this.newcolor+" GradientType="+this.gradient_effect+")"
	...
	"progid:DXImageTransform.Microsoft.Gradient(startColorstr="+this.newcolorCompl+", endColorstr="+this.newcolor+" GradientType="+this.gradient_effect+")"
				...
				if(this.type == 'bg'){
					document.getElementById(div).style.backgroundColor=this.newcolor 
				}else if(this.type == 'txt'){
					//console.log(div+' : '+this.type);
					document.getElementById(div).style.color=this.newcolor 
				}
			}
			//console.log('fadebg timer');
			this.timer=setTimeout(this.obj+".fadebg()",this.speed);
		} 
		else {
